...
export default function HighProfileOutreach({
  ...
  console.log("convinceHighProfileSection", convinceHighProfileSection);
        ...
        <div className="flex flex-col md:flex-row gap-12 items-center">
          <div className="md:w-1/2">
            <h2 className="text-3xl font-bold text-[#2F4858] mb-6">
              {convinceHighProfileSection?.heading}
            </h2>
            <p className="text-lg text-gray-700 mb-6">
              {convinceHighProfileSection?.paragraph}
            </p>
            <ul className="space-y-3">
              {convinceHighProfileSection?.checklist?.map((item, index) => (
